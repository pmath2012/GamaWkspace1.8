...
//		write '--------Refreshments----------------';
	...
	aspect base{
		...
		draw square(12) at:{location.x-2,location.y} color:#maroon;
		draw geometry: 'Casino' color:#black font:font("Helvetica",20,#bold) at:{location.x-5,location.y};
	}
}
...
species walker skills:[moving] control: simple_bdi{
	...
	perceive target:walker in:4{
		float intensity <- get_intensity(myself.love);
		if(intensity>6){
			focus find_love var:location;
		}
	}
	rule belief: new_predicate("gamble") new_desire: get_belief_with_name("gamble");
	rule belief: new_predicate("find_love") new_desire: get_belief_with_name("find_love");
...
//			write 'my money '+money;
	...
	plan look_for_love intention:new_predicate("find_love") {
		color<-#purple;
		point agentLoc <- point(get_current_intention ().values["location_value"] );
		do goto target:point(agentLoc) on:roads;
		if(distance_to(location,agentLoc)<1) {
			write 'reached love';
			do remove_intention(get_current_intention(),true);
